...
export const useMarketData = (feed: MarketFeed, flushMs = 100) => {
  ...
  useEffect(() => {
    ...
    // When the browser tab is backgrounded, Chrome/Edge throttle setInterval
    // to min 1000ms — most 100ms flush ticks are skipped, so the chart falls
    // behind. On re-focus, immediately flush the accumulated data so the UI
    // catches up without a visible gap.

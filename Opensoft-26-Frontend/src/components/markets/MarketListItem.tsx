...
export const MarketListItem: React.FC<{ data: StockData; onSelect?: (symbol: string) => void }> = ({ data, onSelect }) => {
  ...
  return (
    <div className="group flex cursor-pointer items-center gap-3 px-4 py-3 transition-all duration-150 hover:bg-white/[0.03]" onClick={() => onSelect?.(data.symbol)}>
      {/* Logo */}
      <div
        className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-[10px] font-black text-white transition-transform duration-200 group-hover:scale-110 shadow-sm"
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5">
          <span className="truncate text-[13px] font-bold text-[#E2E8F0]">{data.name || data.symbol}</span>
          {data.isDerived && (
        ...
        <span className="block text-[11px] text-slate-500 font-medium">{data.name ? data.symbol : ''}</span>
      </div>

      {/* Sparkline */}
      <svg className="h-[28px] w-[64px] shrink-0" viewBox={`0 0 ${sparkW} ${sparkH}`} preserveAspectRatio="none">
        <path d={sparkPath} fill="none" stroke={lineColor} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" opacity="0.9" />
      </svg>

      {/* Price */}
      <div className="flex w-[90px] shrink-0 flex-col items-end">
        <span className="text-[13px] font-bold text-[#E2E8F0]">
          {data.price.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
        </span>
        <span className="text-[10px] text-slate-500 uppercase">{data.currency}</span>
      </div>

      {/* Change */}
      <div
        className={`w-[66px] shrink-0 text-right text-[12px] font-bold px-2 py-0.5 rounded-md ${
          isPositive ? 'text-[#00C076] bg-[#00C076]/10' : 'text-[#FF4560] bg-[#FF4560]/10'
        }`}
      >

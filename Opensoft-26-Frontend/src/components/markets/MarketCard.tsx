...
export const MarketCard: React.FC<{ data: StockData; onSelect?: (symbol: string) => void; sparkValues?: number[] }> = ({ data, onSelect, sparkValues }) => {
  const [hovered, setHovered] = useState(false);
  const isPositive = data.changePercent >= 0;
  const color = useMemo(() => symbolColor(data.symbol), [data.symbol]);
  const pts = useMemo(() => {
    if (sparkValues && sparkValues.length >= 2) {
      return sparkValues.slice(-30);
    }
    return generatePoints(data.symbol, 30, isPositive);
  }, [data.symbol, isPositive, sparkValues]);
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-3 px-4 pt-4 pb-1 z-10">
        <div
          className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl text-[11px] font-black shadow-lg transition-transform duration-300 group-hover:scale-110"
          style={{
        ...
        <div className="min-w-0 flex-1">
          ...
          <span className="truncate text-[14px] font-bold text-[#D9DEE3]">{data.tokenSymbol || data.symbol}</span>
            {data.isDerived && (
